...
@Telebot.on_message(filters.command('change_role') & filters.private)
def change_role(bot: Telebot, message):
    try:
...
@Telebot.on_callback_query(filters.regex('.*role'))
def final_change_role(bot: Telebot, callback_query: CallbackQuery):
    try:
